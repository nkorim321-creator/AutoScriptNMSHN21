...
(function () {
    ...
    function initMain(){
        ...
        function writeRewardSignal(dollars){
            // Same-browser: GM_setValue (instant)
            try { GM_setValue('mrp_reward_' + _teamKey, dollars.toFixed(2)); } catch(e){}
        ...
        function startRewardPoller(){
            ...
            setInterval(function(){
                try {
                    ...
                    if (!isNaN(dollars)) applyTeamReward(dollars);
                } catch(e){}
            }, 2000);
            ...
            setInterval(function(){
                readRewardSignal(function(dollars){
                    ...
                    // Propagate to same-browser tabs immediately
                    try { GM_setValue(gmKey, dollars.toFixed(2)); } catch(e){}
                });
            }, 3000);
        }
        ...
        function loadState(){
            ...
            state.scanDelay        = 0;     // 0ms — fire next scan immediately, no artificial wait
            state.blastCount       = 3;
            state.maxConcurrent    = 30;
            state.parallelScanners = 7;      // 7 dedicated scanners — page1/2/3 + qualified + reward + fast + size10
        ...
        function fireAccept(groupId, hitInfo, blastCount){
            ...
            function onSucc(body, furl, respUrl){
                ...
                if ((fu.indexOf('/tasks/') > -1 && fu.indexOf('accept_random') === -1) ||
                    ...
                    if (resolved) return;
                    resolved = true; delete acceptLocks[groupId];
                ...
                // Not accepted — do NOT set resolved. Other in-flight blasts may succeed.
                acceptedRecently[groupId] = now();
                setTimeout(function(){ delete acceptedRecently[groupId]; }, 150);
            }
            ...
            for (var b = 0; b < blastCount; b++){
                (function(blastIdx){
                    ...
                    // fetch() fast path — same-origin, no TM bridge overhead
                    // Can detect accept from redirect URL BEFORE reading body (huge speed gain)
                    if (_fetch){
                        var _ac = new AbortController();
                        var _at = setTimeout(function(){ try { _ac.abort(); } catch(e){} }, 5000);
                        _fetch(baseUrl + '?_=' + (now() + blastIdx * 37 + Math.floor(Math.random() * 99991)), {
                            ...
                            if (fu.indexOf('/tasks/') > -1 && fu.indexOf('accept_random') === -1){
                                ...
                                if (!resolved){ resolved = true; delete acceptLocks[groupId]; onAccepted(groupId, hitInfo, ''); }
                                settle(); return;
                            }
                        ...
                        // GM_xmlhttpRequest fallback (when fetch unavailable)
                        GM_xmlhttpRequest({
                            method: 'GET',
                            ...
                            onload: function(r){
                                ...
                                try {
                                    ...
                                    if (fu.indexOf('/tasks/') > -1 && fu.indexOf('accept_random') === -1){
                                        ...
                                        resolved = true; delete acceptLocks[groupId]; onAccepted(groupId, hitInfo, '');
        ...
        function onAccepted(gid, hitInfo, assignId){
            ...
            if (isFav || isBatch){
                // Favorite/batch — 80ms gap only, never permanently blocked
                acceptedRecently[gid] = now();
                setTimeout(function(){ delete acceptedRecently[gid]; }, 80);
            } else if (hitCount > 1){
                ...
                // Safety: clear queuedGroupIds after 30s (syncQueue no longer re-adds)
                setTimeout(function(){ delete queuedGroupIds[gid]; }, 30000);
            }
        }
        ...
        function processResults(results){
            ...
            // If queue is already full, only process batch/fav HITs (they bypass the queue limit)
            var queueFull = runtime.queueFree <= 0;

            // ── INSTANT FIRE: position 0 is always the newest HIT (sort=updated_desc) ──
            // Fire immediately before any sort/filter to minimise latency.
            // Track groupId so the eligible loop below skips it and avoids a double-fire.
            ...
            if (newest && newest.hit_set_id){
                ...
                if (!blockedGroupIds[newestGid] && !isBlocked(newestReqId, newestReqName)){
                    ...
                    // Respect minReward filter — skip instant fire if below threshold
                    var rewardOk = (state.minReward <= 0 || newestReward >= state.minReward);
                    var newestIsBatch = newestCount > 100;
                    if (newestCount > 0 && rewardOk && (!queueFull || newestIsBatch)){
                        var newestInfo = {
                        ...
                        instantFiredGid = newestGid; // mark — skip in eligible loop below
                    }
                } else {
            ...
            if (state.favoriteMode && state.favoriteHIT){
                ...
                if (favFound) fireAccept(state.favoriteHIT.groupId, state.favoriteHIT, 15);
            }
            ...
            results.forEach(function(hit){
                ...
                // Skip non-batch HITs if queue is full — they'll fail with "queue full" anyway
                if (!isBatchHit && queueFull) return;
            ...
            // smartBlasts: match blast count to available slots — avoid overkill 429s
            //  1–3 slots →  8 blasts  (max aggression on scarce HITs)
            //  4–5       →  6 blasts
            //  6–20      → 10 blasts  (common small batch)
            //  21–100    →  6 blasts
            //  101–999   → 12 blasts  (moderate batch — fill queue fast)
            //  1000+     → 20 blasts  (mass batch — grab as many as possible)
        ...
        //
        //  7 scanners × 100ms delay = ~35 requests/sec total
        ...
        //  Scanner 1: page_size=10, unfiltered — fast top-10
        //  Scanner 2: page_size=25, unfiltered — full top-25 coverage
        //  Scanner 3: page_size=25, qualified  — qualified HITs coverage
        //  Scanner 4: page_size=25, page 2     — catches HITs pushed off page 1
        //  Scanner 5: page_size=1, updated_desc — second ultra-fast new-HIT detector
        //  Scanner 6: page_size=25, page 3     — deep sweep
        // ================================================================
        var SCANNER_URLS = [];
        ...
        function buildScannerUrls(){
            ...
            // THREE dedicated page_size=1 scanners at staggered phases (3ms apart).
            // Each cycles independently at ~200ms RTT.
            // Worst-case latency to detect a new HIT = ~67ms (200ms / 3 scanners).
            SCANNER_URLS = [
                p1u,   // 0 — ultra-fast new-HIT detector (phase A)
                p1u,   // 1 — ultra-fast new-HIT detector (phase B, +3ms stagger)
                p1u,   // 2 — ultra-fast new-HIT detector (phase C, +6ms stagger)
                p10u,  // 3 — top-10 newest: catches multiple new HITs in one call
                p25u,  // 4 — full page: batch detection + HITs pushed below position 10
                p25q,  // 5 — qualified: HITs you can definitely accept
                pg2    // 6 — page 2: catches HITs off front page
            ];
        ...
        function getScannerUrl(scannerId){
            ...
            var idx;
            if (scannerId <= 5){
                // Scanners 0-5: fixed dedicated slots
                idx = Math.min(scannerId, SCANNER_URLS.length - 1);
            } else {
                // Scanner 6+: round-robin through all URLs from index 6 onward
                // This ensures pg3 AND all watch-list requester URLs get scanned
                var extraCount = Math.max(1, SCANNER_URLS.length - 6);
                idx = 6 + (_watchRRIdx++ % extraCount);
            ...
            watchdogTimer = setInterval(function(){
                ...
                if (lockKeys.length > 0){
                    lockKeys.forEach(function(g){
                        ...
                        if (!hasInFlight) delete acceptLocks[g];
                    });
                }
            ...
            setInterval(function(){
                readSyncSignal(function(signal){
                    ...
                    if (signal === 'RUN'){
                        ...
                        updateMegaBtn();
                        if (state.autoStartEnabled && !runtime.isRunning && !captchaSystem.captchaActive){
                            startScan();
                        }
                    }
                });
        ...
        function initTeamAndPoller() {
            ...
            detectTeam(_savedWid, function(key, label, rawName) {
                ...
                startGlobalPoller();
                startRewardPoller(); // must start here so reward syncs even if scan not running
            });
        }
